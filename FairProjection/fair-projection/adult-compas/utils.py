...
def MP_tol_ldp(df, tolerance, protected_attrs, label_name, use_protected, use_sample_weight, tune_threshold, log, model='gbm', div='cross-entropy', num_iter=10, rand_seed=42, constraint='meo', epsilon=1.0):
    acc = np.zeros((len(tolerance), num_iter))
    brier = np.zeros((len(tolerance), num_iter))
    auc = np.zeros((len(tolerance), num_iter))
    meo = np.zeros((len(tolerance), num_iter))
    ...
    for seed in tqdm(range(num_iter)):
        ...
        # Compute empirical probabilities P(S=0) and P(S=1)
        s_train = dataset_orig_train[protected_attrs].to_numpy().ravel()
        P_s0 = np.mean(s_train == 0)
        P_s1 = np.mean(s_train == 1)

        # Set probabilities (p, q) based on P(S=0) and P(S=1)
        if P_s0 < P_s1:
            p, q = (np.exp(-epsilon) / 2, 1 / 2)
        else:
            p, q = (1 / 2, np.exp(-epsilon) / 2)

        # Perturb sensitive attribute S in the training dataset
        perturbed_s_train = s_train.copy()
        for i in range(len(s_train)):
            if s_train[i] == 0:
                # Flip S=0 with probability p
                perturbed_s_train[i] = np.random.choice([0, 1], p=[1-p, p])
            else:
                # Flip S=1 with probability q
                perturbed_s_train[i] = np.random.choice([1, 0], p=[1-q, q])

        # Now, replace the original sensitive attribute with the perturbed one
        if len(protected_attrs) == 1:
            dataset_orig_train[protected_attrs[0]] = perturbed_s_train
        else:
            perturbed_s_train = perturbed_s_train.reshape(-1, len(protected_attrs))
            dataset_orig_train[protected_attrs] = perturbed_s_train


       

        # Convert the dataset to AIF360 StandardDataset format
        dataset_orig_train = StandardDataset(dataset_orig_train, label_name=label_name, favorable_classes=[1],
                                             protected_attribute_names=protected_attrs, privileged_classes=[[1]])
        dataset_orig_test = StandardDataset(dataset_orig_test, label_name=label_name, favorable_classes=[1],
                                            protected_attribute_names=protected_attrs, privileged_classes=[[1]])
        ...
        X_train, y_train = dataset_orig_train.features[:, idx_features], dataset_orig_train.labels.ravel()
        X_test, y_test = dataset_orig_test.features[:, idx_features], dataset_orig_test.labels.ravel()
        s_train = dataset_orig_train.features[:, idx_protected].ravel()  # perturbed s_train
        s_test = dataset_orig_test.features[:, idx_protected].ravel()  # unchanged s_test

        # Declare classifiers
        if model == 'gbm':
            clf_YgX = GradientBoostingClassifier(random_state=rand_seed)  # Predict Y from X
            clf_SgX = GradientBoostingClassifier(random_state=rand_seed)  # Predict S from X (needed for SP)
            clf_SgXY = GradientBoostingClassifier(random_state=rand_seed)  # Predict S from (X,Y)
        elif model == 'logit':
            clf_YgX = LogisticRegression(random_state=rand_seed, max_iter=10000)  # Predict Y from X
            clf_SgX = LogisticRegression(random_state=rand_seed, max_iter=10000)  # Predict S from X (needed for SP)
            clf_SgXY = LogisticRegression(random_state=rand_seed, max_iter=10000)  # Predict S from (X,Y)
        elif model == 'rfc':
            clf_YgX = RandomForestClassifier(random_state=rand_seed, n_estimators=10, min_samples_leaf=10)  # Predict Y from X
            clf_SgX = RandomForestClassifier(random_state=rand_seed, n_estimators=10, min_samples_leaf=10)  # Predict S from X (needed for SP)
            clf_SgXY = RandomForestClassifier(random_state=rand_seed, n_estimators=10, min_samples_leaf=10)  # Predict S from (X,Y)
        else:
            log.write('Error: Undefined Model\n')
            log.flush()
            return
        ...
        t_fit = time.localtime()
        # Initialize GFair class and train classifiers
        gf = GF.GFair(clf_YgX, clf_SgX, clf_SgXY, div=div)
        if use_sample_weight:
            gf.fit(X=X_train, y=y_train, s=s_train, sample_weight=dataset_orig_train.instance_weights)
        else:
        ...
        for i, tol in enumerate(tolerance):
            ...
            try:
                ...
                # Evaluation
                evaluation(i, seed, gf, X_test, y_test, s_test, threshold, acc, brier, auc, meo, meo_abs, mo, sp)
