from exp import Exp
import pandas as pd
import numpy as np
from stats import is_larger
import time
from scipy import stats
import os
# === import your OPT mechanism ===
from opt_preprocessor import OPTPreprocessor


def mean_ci(values, confidence=0.95):
    arr = np.asarray(values, dtype=float)
    n = len(arr)
    mean = np.mean(arr)
    # use sample std (ddof=1) and t critical for small n
    sem = arr.std(ddof=1) / np.sqrt(n)
    tcrit = stats.t.ppf((1 + confidence) / 2.0, df=n - 1)
    ci = tcrit * sem
    return mean, ci

    
def run(data, repeat=10, seed=1, epsilon=1):
    np.random.seed(seed)

    treatments = ["None", "FairBalance", "OPT"]

    metrics = ["Accuracy", "AUC", "mEOD", "mAOD", "smEOD", "smAOD", "Runtime"]
    columns = ["Treatment"] + metrics
    test_result = {column: [] for column in columns}
    train_result = {column: [] for column in columns}

    for treat in treatments:
        test_result["Treatment"].append(treat)
        train_result["Treatment"].append(treat)

        twopart = treat.split("+")
        if len(twopart) < 2:
            if treat.split("-")[0] == "fairsmote":
                treat_predata = treat
                treat_preprocess = "None"
            else:
                treat_preprocess = treat
                treat_predata = "None"
        else:
            treat_preprocess = twopart[0]
            treat_predata = twopart[1]

        test_accuracy, test_auc, test_eod, test_aod, test_smeod, test_smaod, test_time = ([] for _ in range(7))
        train_accuracy, train_auc, train_eod, train_aod, train_smeod, train_smaod, train_time = ([] for _ in range(7))

        for i in range(repeat):
            t1 = time.time()

            if treat == "OPT":
                exp = Exp(data=data, treatment="OPT", predata="None", inject=epsilon)  # inject optional
                m_train, m_test = exp.one_exp(epsilon_opt=epsilon, random_state_opt=i)
                runtime = time.time() - t1
            else:
                # === existing FairBalance and other methods ===
                exp = Exp(data=data, treatment=treat_preprocess, predata=treat_predata)
                m_train, m_test = exp.one_exp()
                runtime = time.time() - t1

            # === record metrics ===
            test_accuracy.append(m_test.accuracy())
            test_auc.append(m_test.auc())
            test_eod.append(m_test.eod())
            test_aod.append(m_test.aod())
            test_smeod.append(m_test.seod())
            test_smaod.append(m_test.saod())
            test_time.append(runtime)

            train_accuracy.append(m_train.accuracy())
            train_auc.append(m_train.auc())
            train_eod.append(m_train.eod())
            train_aod.append(m_train.aod())
            train_smeod.append(m_train.seod())
            train_smaod.append(m_train.saod())
            train_time.append(runtime)

        # collect all lists
        for key, lst in zip(metrics,
                            [test_accuracy, test_auc, test_eod, test_aod,
                             test_smeod, test_smaod, test_time]):
            test_result[key].append(lst)
        for key, lst in zip(metrics,
                            [train_accuracy, train_auc, train_eod, train_aod,
                             train_smeod, train_smaod, train_time]):
            train_result[key].append(lst)

    # === ranking and summarizing (TEST) ===
    for key in metrics:
        if key in ["Accuracy", "AUC"]:
            rank = ranking(test_result[key], better="higher")
        else:
            rank = ranking(test_result[key], better="lower")

        pretty = []
        for i in range(len(test_result[key])):
            mean, ci = mean_ci(test_result[key][i], confidence=0.95)
            pretty.append(f"r{rank[i]}: {mean:.3f} ± {ci:.3f}")
        test_result[key] = pretty

    # === ranking and summarizing (TRAIN) ===
    for key in metrics:
        if key in ["Accuracy", "AUC"]:
            rank = ranking(train_result[key], better="higher")
        else:
            rank = ranking(train_result[key], better="lower")

        pretty = []
        for i in range(len(train_result[key])):
            mean, ci = mean_ci(train_result[key][i], confidence=0.95)
            pretty.append(f"r{rank[i]}: {mean:.3f} ± {ci:.3f}")
        train_result[key] = pretty

    # ensure output dirs exist
    os.makedirs("results/test", exist_ok=True)
    os.makedirs("results/train", exist_ok=True)

    df_test = pd.DataFrame(test_result, columns=columns)
    df_test.to_csv("results/test/" + data + ".csv", index=False)

    df_train = pd.DataFrame(train_result, columns=columns)
    df_train.to_csv("results/train/" + data + ".csv", index=False)


def ranking(result, better="lower"):
    if better == "higher":
        result = (1.0 - np.array(result)).tolist()
    medians = [np.median(r) for r in result]
    order = np.argsort(medians)
    rankings = [0] * len(order)
    rank = 0
    pre = []
    for i, idx in enumerate(order):
        if i == 0:
            pre.extend(result[idx])
        else:
            diff = is_larger(result[idx], pre)
            if diff > 1:
                rank += 1
                pre = result[idx]
        rankings[idx] = rank
    return rankings


if __name__ == "__main__":
    datasets = ["bank", "german", 
                "student-mat"]
    for data in datasets:
        run(data, seed=1, epsilon=1)
